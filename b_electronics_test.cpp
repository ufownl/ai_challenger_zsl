...
using testing_net = loss_multiattr<dlib::sig<base_network<75>::testing_type>>;

int main(int argc, char* argv[]) {
  dlib::set_dnn_prefer_smallest_algorithms();
  auto images = dlib::directory{"ai_challenger_zsl2018_train_test_b_20180321/zsl_b_electronics_test_20180321"}.get_files();
  std::ifstream attr_in("ai_challenger_zsl2018_train_test_b_20180321/zsl_b_electronics_train_20180321/zsl_b_electronics_train_annotations_attributes_per_class_20180321.txt");
  if (!attr_in) {
    std::cerr << "Could not load the testing attributes." << std::endl;
    return 1;
  }
  auto attributes = load_label_attributes(attr_in, 75);
